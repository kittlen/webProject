...
function table() {
	$.ajax({
		url : "VOptCourseScheduleMgrServlet?judge=TSse",
		type : "post",
		dataType : "json",
		success : function(data) {
			...
			for (var i = 0, t = 0; i < data.length; i++) {
				...
				if(Thtml!=''){
				$("#TimeTablesTable tr").eq(ClassTime).find("td").eq(Classfestival).html(Thtml+'</br>'+
						data[i].course+" "+data[i].classRoomName+'</br>教师名称:'+data[i].teacherName); // 将表格第m+1行第n+1列的内容设置为s
			}else{
				$("#TimeTablesTable tr").eq(ClassTime).find("td").eq(Classfestival).html(data[i].course+
						" "+data[i].classRoomName+'</br>教师名称:'+data[i].teacherName); // 将表格第m+1行第n+1列的内容设置为s
			}}
