...
function table() {
	...
	$.ajax({
				url : "VOptCourseScheduleMgrServlet?judge=Tse",
				type : "post",
				dataType : "json",
...
function OnSuccess(index,CourseScheduleID) {
	...
	$("[name=StudentName]").val(table.rows[index].cells[2].innerHTML);
	$("[name=Stu_ID]").val(table.rows[index].cells[3].innerHTML);
	$("[name=ScoreOne]").val(table.rows[index].cells[7].innerHTML);
	$("[name=ScoreTwo]").val(table.rows[index].cells[8].innerHTML);
	$("[name=ScoreThree]").val(table.rows[index].cells[9].innerHTML);
	$("[name=CourseScheduleID]").val(CourseScheduleID);
	$("[name=StudentName]").attr("readOnly", true);
...
function inputreset() {
	...
	$("[name=ScoreTwo]").val();
	$("[name=ScoreThree]").val();
	$("[name=CourseScheduleID]").val();
	$("[name=StudentName]").attr("readOnly", false);
