...
public class TeacherPhoto extends ActionSupport {
	...
	public String execute() throws Exception {
		...
		String dirname = "Teacher\\teacherphoto";// 图片文件夹

		HttpServletRequest request = ServletActionContext.getRequest();
		Teacher teacher = (Teacher) request.getSession().getAttribute("Tlogin");// 获取对象
		imageFileName = teacher.getPhoto();// 设置文件名为对象照片字段
		...
		/*
		 * String fi
		 * =request.getSession().getServletContext().getRealPath("/")+dirname;
		 */// 另一种获取路径方式

		String type = "."
				+ imageContentType
						.substring(imageContentType.lastIndexOf("/") + 1);// 获取文件类型
