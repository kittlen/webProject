...
public class StudentPhoto extends ActionSupport {
	...
	public String execute() throws Exception {
		...
		String dirname="Student\\studentphoto";//图片文件夹
		
		HttpServletRequest request = ServletActionContext.getRequest();
		Student student = (Student) request.getSession().getAttribute("Slogin");// 获取对象
		imageFileName = student.getPhoto();// 设置文件名为对象照片字段
		
		
		String realpath = ServletActionContext.getServletContext().getRealPath(
				"/")+dirname;// 获取服务器所在路径+设置文件所在路径*/
		/*String fi =request.getSession().getServletContext().getRealPath("/")+dirname;*///另一种获取路径方式
