...
function conf(TeacherID) {
	if (confirm("确认删除此条信息?")) {
		location = 'Teacher/delete?TeacherID=' + TeacherID;
	}
}
function confPwd(TeacherID) {
	if (confirm("确认重置本账号密码?")) {
		location = 'Teacher/reset?TeacherID=' + TeacherID;
	}
}
...
function update(index) {
	...
	document.forms.action = "Teacher/update";
}
function inputreset() {
	$("[name=TeacherID]").val();
	$("[name=Name]").val();
	...
	document.forms.action = "Teacher/insert";
}